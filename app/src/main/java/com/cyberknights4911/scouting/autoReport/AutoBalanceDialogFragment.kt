 ...
 */
@AndroidEntryPoint
class AutoBalanceDialogFragment : BottomSheetDialogFragment() {
    private val autoBalanceViewModel: AutoBalanceViewModel by viewModels()
    ...
    override fun onCreateView(
        ...
        with (binding) {
            ...
            balance.setOnCheckedChangeListener { _, id ->
                autoBalanceViewModel.balance.postValue(
                    when (id) {
            ...
            balanceOkay.setOnClickListener {
                autoBalanceViewModel.saveReport()
                findNavController().navigateUp()
            }
        }

        autoBalanceViewModel.initialReport.observe(viewLifecycleOwner) { initialReport ->
            Log.d("AutoBalanceDialogFragment", "report fetched ${initialReport.reportId}")
            binding.root.visibility = View.VISIBLE
        }

        autoBalanceViewModel.balance.observe(viewLifecycleOwner) { balance ->
            when (balance) {
                Balance.ENGAGED -> {
